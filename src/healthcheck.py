"""Long-running RPC poller for the Arcium node. Telegram alert on stall."""
...
def main():
    ...
    p.add_argument("--rpc", default=os.getenv("ARCIUM_RPC","127.0.0.1:9001"))
    ...
    while True:
        try:
            h = rpc(args.rpc, "clusterheight").get("result", 0)
            c = rpc(args.rpc, "ceremonies").get("result", 0)
            print(f"[{time.strftime('%H:%M:%S')}] height={h} ceremonies={c}")
            if h == last_h and c == 0:
                msg = f"⚠️ Arcium stalled: h={h} ceremonies=0"
                print(msg)
            ...
            if fails >= 3 and args.tg_bot:
                send_tg(args.tg_bot, args.tg_chat, f"❌ Arcium RPC down 3x: {e}")
